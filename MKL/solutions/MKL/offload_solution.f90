...
program getting_started
  ...
  if ('f' == prec_type) then ! single-precision
      ...
      !dir$ attributes offload:mic :: srun
      !dir$ offload target(mic) &
        & out(fresults:length(isize)) out(fexpect:length(isize)) &
        & in(fa:length(size(fa))) in(fb:length(size(fb)))
      ...
      !dir$ attributes offload:mic :: drun
      !dir$ offload target(mic) &
        & out(dresults:length(isize)) out(dexpect:length(isize)) &
        & in(da:length(size(da))) in(db:length(size(db)))
...
!dir$ attributes offload:mic :: sgemm_fortran
...
!dir$ attributes offload:mic :: dgemm_fortran
...
!dir$ attributes offload:mic :: sgemm_mkl
...
!dir$ attributes offload:mic :: dgemm_mkl
...
!dir$ attributes offload:mic :: sgemm_fortran
!dir$ attributes offload:mic :: sgemm_mkl
!dir$ attributes offload:mic :: dsecnd
